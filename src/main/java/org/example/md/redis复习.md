...
热点 key 问题是指在 Redis 中，某些 key 的访问频率远高于其他 key，从而影响到整体性能

常用解决方法：

1. 热点key拆分，把数据分散到多个 key 中（例如：key1，key2，key3拥有相同的数据，用户访问不同的key。也能把这3个key各存一部分数据，一些公共数据每个用户看到的都不一样（例如推流信息））
2. 多级缓存，使用本地缓存 + redis缓存
3. 读写分离，采用**主从复制**的方式来分担redis压力（主节点写，从节点读（从多个从节点））
4. 限流和降级，对热点数据进行**限流**访问，或者对返回的数据进行降级处理（降级数据或空值）

发现热点数据的方法：

1. 业务分析
2. 集群监控
3. hotkey参数监控 （redis-cli --hotkeys）redis自带，但是是扫描全部keyspace
4. monitor命令，自带，耗性能50%
5. 客户端收集（查询redis做统计，统计数据发送给**聚合运算平台**）
6. 代理层收集（定制个代理层进行数据转发和统计)

### Redis 集群的实现原理是什么

Redis 集群通过分片（sharding）和复制（replication）来实现数据的分布式存储和高可用性。
客户端发送请求时，会访问redis的**任意节点**，如果节点存储了数据则返回，没有则根据**请求键计算哈希槽**
找到正确的节点（哈希槽（把键空间分为16384(2^14)个槽），采用分片的方式，每个主节点有自己的槽范围）

节点之间通过**Gossip协议**(流行病协议)进行通信，维护集群状态，包括节点信息、槽分配信息等。
工作原理以下：

1. 状态报告：在特定时间内**随机**给其他节点发送自己的状态信息（主从关系，槽位分布）
2. 更新信息：接收到数据把数据更新到自己节点，**传播**给其他节点
3. 节点检测：**周期性**交换节点信息，如果节点在时间内没响应则标为**故障节点**
4. 容错处理：检测到故障节点，其他节点可以采用重新分配策略

key通过C|RC16算法计算哈希值，然后对16384取模得到哈希槽，根据哈希槽找到对应的节点。（ CRC16(key) % 16384
求出槽点值，根据槽点范围确定节点 ）

**客户端并不是直接连接到key对应的节点**：

1. 随机连接一个节点，计算key的哈希槽
2. 该节点返回`moved`错误（里面包含需要连接的节点的ip和端口）
3. 客户端跟对应新节点建立连接

### Redis 中的 Big Key 问题是什么？如何解决？
Big Key 问题是指在 Redis 中，某些 key 的数据量非常大，导致内存占用过高，影响 Redis 的性能和稳定性。
1. redis是单线程，big key比较耗时，可能会造成阻塞
2. big key 对资源占用大，产生的网络流量大，可能造成网络传输延长
3. 耗时太长，造成连接超时
4. 集群模式下，计算的槽在同一个节点，造成分布不均匀，查询效率低

解决方法：
- 拆分：将 big key 拆分成多个小 key
- 使用合适的数据结构
- 使用压缩算法（业务代码上）
- 存常用的数据

`redis-cli --bigkeys`命令查看较大的key(内置方法)
